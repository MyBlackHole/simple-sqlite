...
int sqliteBtreeOpen(
	const char
		*zFilename, /* Name of the file containing the BTree database */
...
int sqliteBtreeCursor(Btree *pBt, int iTable, int wrFlag, BtCursor **ppCur)
{
	...
	rc = sqlitepager_get(pBt->pPager, pCur->pgnoRoot,
			     (void **)&pCur->pPage);
	if (rc != SQLITE_OK) {
		goto create_cursor_exception;
	}
...
static int getPayload(BtCursor *pCur, int offset, int amt, char *zBuf)
{
	...
	while (amt > 0 && nextPage) {
		OverflowPage *pOvfl;
		rc = sqlitepager_get(pCur->pBt->pPager, nextPage,
				     (void **)&pOvfl);
		if (rc != 0) {
			return rc;
		}
...
static int allocatePage(Btree *pBt, MemPage **ppPage, Pgno *pPgno)
{
	...
	if (pPage1->freeList) {
		...
		pPage1->nFree--;
		// 获取第一个可用页面 pOvfl
		rc = sqlitepager_get(pBt->pPager, pPage1->freeList,
				     (void **)&pOvfl);
		if (rc)
			return rc;
		...
		if (pInfo->nFree == 0) {
			...
			*pPgno = pInfo->aFree[pInfo->nFree];
			rc = sqlitepager_get(pBt->pPager, *pPgno,
					     (void **)ppPage);
			sqlitepager_unref(pOvfl);
			if (rc == SQLITE_OK) {
...
static int freePage(Btree *pBt, void *pPage, Pgno pgno)
{
	...
	if (pPage1->nFree > 0 && pPage1->freeList) {
		...
		if (rc == SQLITE_OK) {
			FreelistInfo *pInfo =
				(FreelistInfo *)pFreeIdx->aPayload;
			if (pInfo->nFree <
			    (sizeof(pInfo->aFree) / sizeof(pInfo->aFree[0]))) {
				rc = sqlitepager_write(pFreeIdx);
				if (rc == SQLITE_OK) {
					pInfo->aFree[pInfo->nFree] = pgno;
					pInfo->nFree++;
					sqlitepager_unref(pFreeIdx);
					sqlitepager_dont_write(pBt->pPager,
							       pgno);
					return rc;
				}
			}
...
static int balance(Btree *pBt, MemPage *pPage, BtCursor *pCur)
{
		...
		if (pPage->nCell == 0) {
			if (pPage->u.hdr.rightChild) {
				...
				*/
				...
				rc = sqlitepager_get(pBt->pPager, pgnoChild,
						     (void **)&pChild);
				if (rc)
					return rc;
				...
				pPage->isInit = 0;
				rc = initPage(pPage,
					      sqlitepager_pagenumber(pPage), 0);
				assert(rc == SQLITE_OK);
	...
	*/
	...
	for (i = 0, k = nxDiv; i < 3; i++, k++) {
		...
		rc = sqlitepager_get(pBt->pPager, pgnoOld[i],
				     (void **)&apOld[i]);
		if (rc)
	...
	*/
	if (pCur) {
		...
		for (i = 0; i < nOld; i++) {
			...
			iCur += apOld[i]->nCell;
			if (i < nOld - 1 && pCur->pPage == pParent &&
			    pCur->idx == idxDiv[i]) {
				break;
			}
			iCur++;
	...
	*/
	...
	if (pCur) {
		if (j <= iCur && pCur->pPage == pParent &&
		    pCur->idx > idxDiv[nOld - 1]) {
...
int sqliteBtreePageDump(Btree *pBt, int pgno, int recursive)
{
	...
	while (idx > 0 && idx <= SQLITE_PAGE_SIZE - MIN_CELL_SIZE) {
		...
		printf("cell %2d: i=%-10s chld=%-4d nk=%-4d nd=%-4d payload=%s\n",
		       i, range, (int)pCell->h.leftChild, NKEY(pCell->h),
		       NDATA(pCell->h), payload);
...
)
{
	...
	while (N-- > 0) {
		OverflowPage *pOvfl;
		if (iPage < 1) {
			sprintf(zMsg, "%d pages missing from overflow list",
				N + 1);
		...
		if (checkRef(pCheck, iPage, zContext))
			break;
		if (sqlitepager_get(pCheck->pPager, (Pgno)iPage,
				    (void **)&pOvfl)) {
...
)
{
	...
	*/
	...
	if ((rc = sqlitepager_get(pCheck->pPager, (Pgno)iPage,
				  (void **)&pPage)) != 0) {
	...
	for (i = 0; i < pPage->nCell; i++) {
		...
		*/
		...
		if (sz > MX_LOCAL_PAYLOAD) {
			int nPage =
				(sz - MX_LOCAL_PAYLOAD + OVERFLOW_SIZE - 1) /
				OVERFLOW_SIZE;
		...
		*/
		...
		d2 = checkTreePage(pCheck, pgno, pPage, zContext, zKey1, nKey1,
				   zKey2, nKey2);
		if (i > 0 && d2 != depth) {
	...
	*/
	...
	for (i = 0; i < SQLITE_PAGE_SIZE; i++) {
		if (hit[i] == 0) {
			sprintf(zMsg, "Unused space at byte %d of page %d", i,
				iPage);
			checkAppendMsg(pCheck, zMsg, 0);
			break;
		} else if (hit[i] > 1) {
			sprintf(zMsg, "Multiple uses for byte %d of page %d", i,
				iPage);
			checkAppendMsg(pCheck, zMsg, 0);
			break;
		}
...
char *sqliteBtreeIntegrityCheck(Btree *pBt, int *aRoot, int nRoot)
{
	...
	if (lockBtree(pBt) != SQLITE_OK) {
		return sqliteStrDup(
			"Unable to acquire a read lock on the database");
	}
	sCheck.pBt = pBt;
	...
	*/
	for (i = 0; i < nRoot; i++) {
		if (aRoot[i] == 0)
			continue;
		checkTreePage(&sCheck, aRoot[i], 0, "List of tree roots: ", 0,
			      0, 0, 0);
	}
