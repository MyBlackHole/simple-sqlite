...
void sqliteHashInit(Hash *new, int keyClass, int copyKey)
{
	...
	new->copyKey = copyKey && (keyClass == SQLITE_HASH_STRING ||
				   keyClass == SQLITE_HASH_BINARY);
	new->first = 0;
	new->count = 0;
	new->htsize = 0;
	new->ht = 0;
...
*/
static int (*compareFunction(int keyClass))(const void *, int, const void *,
					    int)
{
	switch (keyClass) {
	case SQLITE_HASH_INT:
...
)
{
	...
	int (*xCompare)(const void *, int, const void *,
	...
	if (pH->ht) {
		...
		while (count-- && elem) {
			if ((*xCompare)(elem->pKey, elem->nKey, pKey, nKey) ==
			    0) {
				return elem;
			}
			elem = elem->next;
